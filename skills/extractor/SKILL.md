...
- **姓名**：角色全名
- **角色定位**：主角/配角/龙套
- **外貌描写（appearance）**：只写可直接画出来的视觉信息，包括年龄段、性别感、体型、面部特征、发型、着装、配饰、神情和姿态
- **性格特点（personality）**：只写能转化为表情和气质的短标签，例如疲惫但镇定、克制沉稳、冷峻警觉
- **角色描述（description）**：只保留会影响视觉识别的身份与气质，例如宗门身份、职业、阶层、整体气场；不要写剧情经历、关系脉络、系统能力、道具获得或本集事件

## 角色图字段边界

- `appearance / description / personality` 会参与角色出图提示词，必须保持画面可用
- 不要把“穿越者、系统、权限、秘境、玉牌、某一集发生的事件”等不可视剧情信息写入这些字段
- 剧情设定只能作为判断角色重要性的依据，不要沉淀到出图字段
