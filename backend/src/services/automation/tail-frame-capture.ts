...
import { resolveAutomationProjectRoot, resolveTailFrameInputPath } from './tail-frame-path-policy.js'
...
export async function captureAndPersistTailFrame(videoGenerationId: number, localPath: string | null | undefined): Promise<string | null> {
  const inputAbs = resolveTailFrameInputPath(localPath, DATA_ROOT, STORAGE_ROOT)
