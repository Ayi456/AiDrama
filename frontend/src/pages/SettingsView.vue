...
import {
  ...
  settingsProviders,
  settingsServiceMeta,
  settingsServiceTypes,
  stringifySettings,
} from './settings-ai-service-policy'
...
const presetsByType = listProviderPresets
...
function applyProviderPreset(type, provider) {
  const preset = presetsByType(type).find(item => item.provider === provider)
  if (!preset) return
