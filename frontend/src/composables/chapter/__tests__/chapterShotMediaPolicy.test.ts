...
runTest('default video prompt adds shot boundary, end-state and risky motion guardrails', () => {
  ...
  assert.match(prompt, /入场与首帧/)
  assert.match(prompt, /主体与场景/)
  assert.match(prompt, /分秒时间轴/)
  assert.match(prompt, /0\.0-0\.5秒/)
  assert.match(prompt, /14\.5-15\.0秒/)
  assert.match(prompt, /运镜与画面/)
  assert.match(prompt, /固定中景/)
  assert.match(prompt, /出场与尾帧：电梯到达一楼，门即将打开/)
  assert.match(prompt, /声音与对白/)
  assert.match(prompt, /失败降级/)
  ...
  assert.doesNotMatch(prompt, /动作阶段/)
